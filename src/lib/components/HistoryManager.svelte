...
export function updateUrl(params?: Partial<NavigationParams>) {
	...
	try {
		...
		if (newUrl !== lastUrl) {
			...
			console.log('🔄 HistoryManager: URL unchanged, skipping update:', newUrl);
		}
	} catch (error) {
...
$effect(() => {
	...
	const hasParams = params.batchId !== undefined || params.categoryId || params.storyIndex !== undefined || params.dataLang;
...
$effect(() => {
	...
	const shouldUpdate = expectedUrl !== lastUrl && 
	                    expectedUrl !== currentUrl && 
	                    !currentHasStoryIndex;
	
	if (shouldUpdate) {
		console.log('🔄 HistoryManager: Batch/category state changed, updating URL:', {
			from: lastUrl,
			current: currentUrl,
			to: expectedUrl,
			batchId,
